...
def build_model(num_timesteps):
    ...
    spikes = Input(shape=(num_timesteps,), dtype='int32', name='spikes')

    # Embed spikes into vector space.
    spikes_emb = Embedding(7, 7, init='orthogonal')(spikes)

    # Embed dataset into vector space.
    flat = Flatten()(RepeatVector(num_timesteps)(dataset))
    data_emb = Embedding(10, 10, init='orthogonal')(flat)
    ...
    hidden = merge([calcium_hid, spikes_emb, data_emb],
    ...
    # Builds the model.
    model = Model(input=[dataset, calcium, spikes], output=[output])
...
if __name__ == '__main__':
    ...
    def _grouper():
        ...
        while True:
            ...
            yield ([np.asarray(batched[i]) for i in range(3)],
                   [np.asarray(batched[3]),])
    ...
    class_weight = {
        0: 0.01,  # No spikes, weight way less (doesn't have much information).
        1: 1.,  # Weight the other ones approximately according to their freq.
        2: 10.,
        3: 10.,  # Only worry about getting 1 and 2 correctly classified.
        4: 10.,
        5: 10.,
        6: 10.,
    }
